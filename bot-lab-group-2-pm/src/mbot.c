...
bool timer_cb(repeating_timer_t *rt)
{
    ...
    if (comms_get_topic_data(MBOT_TIMESYNC, &received_time))
    {
        ...
        // Position base odometry 
        delta_d = 0.5*enc2meters*(float)(current_encoders.right_delta + current_encoders.left_delta);
        delta_theta = 1/WHEEL_BASE*enc2meters*(float)(current_encoders.right_delta - current_encoders.left_delta);
        ...
        // float dt = MAIN_LOOP_PERIOD; // time since last reading in seconds

        

        // float vel_R = enc2meters*(float)current_encoders.right_delta/dt;
        // float vel_L = enc2meters*(float)current_encoders.left_delta/dt;

        // float turn_r = WHEEL_BASE/2 * (vel_R + vel_L)/(vel_R - vel_L);
        // float omega = (vel_R - vel_L) / WHEEL_BASE;


        // float CRx = current_odom.x - turn_r*sin(current_odom.theta);
        // float CRy = current_odom.y + turn_r*cos(current_odom.theta);

        // // Straight line motion so vel_R = vel_L, delta_theta = 0
        // if (isinf(turn_r))
        // {
        //     delta_d = vel_R*dt;
        //     current_odom.x += delta_d * cos(current_odom.theta);
        //     current_odom.y += delta_d * sin(current_odom.theta);
        // }
        // else
        // {        
        //     current_odom.x = cos(omega*dt)* (current_odom.x - CRx) - sin(omega*dt)*(current_odom.y - CRy) + CRx;
        //     current_odom.y = sin(omega*dt)* (current_odom.x - CRx) + cos(omega*dt)*(current_odom.y - CRy) + CRy;
        // }

        //Gyrodometry
        delta_theta_gyro = current_imu.tb[2] - prev_imu.tb[2]; //Need to take difference between current and previous :/
        delta_theta_enc = delta_theta;
        delta_go = delta_theta_gyro - delta_theta_enc;
        if (abs(delta_go) > GO_thresh)
        {
            delta_theta = delta_theta_gyro; //Gyro is generally more reliable at larger values
        } 
        else 
        {
            delta_theta = delta_theta_enc; //Gyro bias makes it bad at small values, so default to encoder for these
        }

        current_odom.theta += delta_theta;

        if (current_odom.theta > 2*PI)
        {
            current_odom.theta = current_odom.theta - 2*PI;
        }
        else if (current_odom.theta < 0)
        {
             current_odom.theta = current_odom.theta + 2*PI;
        }
        ...
        if (comms_get_topic_data(MBOT_MOTOR_COMMAND, &current_cmd))
        {
            ...
            if (OPEN_LOOP)
            {
                ...
                float *duty;
                left_sp = current_cmd.trans_v - WHEEL_BASE/2*current_cmd.angular_v; // m/s
                right_sp = current_cmd.trans_v + WHEEL_BASE/2*current_cmd.angular_v; // m/s
                duty = open_loop_control(left_sp, right_sp);
                l_duty = duty[0];
                r_duty = duty[1];

                // Probably a sign issue if this doesn't work
                /*************************************************************
                 * End of TODO
                 *************************************************************/
            }
            else
            {
                 ...
                 *      - Compute the new forward and rotation setpoints that w ill be used in
                ...
                float fwd_err, turn_err; // error in fwd and turn speeds (m/s and rad/s)
                float left_sp_err, right_sp_err; // error in left and right speeds in m/s
                float left_set, right_set; // set points for second PID controller, based on output of first PID controller
                float R_CLsig, L_CLsig; //Signals to send to motor due to closed loop controller

                //Retrieve motor commands
                fwd_sp = current_cmd.trans_v; turn_sp = current_cmd.angular_v;               

                //Compute wheel velocities in m/s
                measured_vel_r = enc2meters*(float)current_encoders.right_delta/dt;
                measured_vel_l = enc2meters*(float)current_encoders.left_delta/dt;
                
                //Compute current fwd and turn velocities from left and right wheel speeds
                measured_vel_fwd = 0.5*(measured_vel_r + measured_vel_l);
                measured_vel_turn = 1/WHEEL_BASE*(measured_vel_r - measured_vel_l);

                //Compute setpoint left and right wheel speeds 
                left_sp = fwd_sp - WHEEL_BASE/2*turn_sp;
                right_sp = fwd_sp + WHEEL_BASE/2*turn_sp;

                //Filter out low frequency noise on setpoints and measurements
                measured_vel_fwd = rc_filter_march(&meas_fwd_vel_lp, measured_vel_fwd);
                measured_vel_turn = rc_filter_march(&meas_turn_vel_lp, measured_vel_turn);
                fwd_sp = rc_filter_march(&set_fwd_vel_lp, fwd_sp);
                turn_sp = rc_filter_march(&set_turn_vel_lp, turn_sp);
                left_sp = rc_filter_march(&set_left_lp, left_sp);
                right_sp = rc_filter_march(&set_right_lp, right_sp);

                //Compute fwd/turn error
                fwd_err = fwd_sp - measured_vel_fwd;
                turn_err = turn_sp - measured_vel_turn;

                //Pass fwd/turn error through PID
                fwd_err = rc_filter_march(&fwd_vel_pid, fwd_err);
                turn_err = rc_filter_march(&turn_vel_pid, turn_err);

                //Compute left and right setpoints for second PID
                left_set = fwd_err - WHEEL_BASE/2*turn_err;
                right_set = fwd_err + WHEEL_BASE/2*turn_err;

                //Compute L/R for second PID
                left_sp_err = left_sp - measured_vel_l;
                right_sp_err = right_sp - measured_vel_r;

                //Pass L/R error through PID
                left_sp_err = rc_filter_march(&left_pid, left_sp_err);
                right_sp_err = rc_filter_march(&right_pid, right_sp_err);

                //Testing single PID
                L_CLsig = left_sp_err;
                R_CLsig = right_sp_err;

                float *duty_OL;
                duty_OL = open_loop_control(left_sp, right_sp);
                l_duty = duty_OL[0] + L_CLsig;
                r_duty = duty_OL[1] + R_CLsig;
                /**
...
int main()
{
     ...
     *
     *************************************************************/
    //Init PID filter for left and right motors
    left_pid = rc_filter_empty(); right_pid = rc_filter_empty();
    rc_filter_pid(&left_pid, left_pid_params.kp, left_pid_params.ki, left_pid_params.kd, 1.0 / left_pid_params.dFilterHz, 1.0 / MAIN_LOOP_HZ);
    rc_filter_pid(&right_pid, right_pid_params.kp, right_pid_params.ki, right_pid_params.kd, 1.0 / right_pid_params.dFilterHz, 1.0 / MAIN_LOOP_HZ);

    //Init lowpass filters for setpoints and measurements
    meas_fwd_vel_lp = rc_filter_empty(); meas_turn_vel_lp = rc_filter_empty();
    set_fwd_vel_lp = rc_filter_empty(); set_turn_vel_lp = rc_filter_empty();
    set_left_lp = rc_filter_empty(); set_right_lp = rc_filter_empty();
    rc_filter_first_order_lowpass(&meas_fwd_vel_lp, 1.0/ MAIN_LOOP_HZ, 0.04);
    rc_filter_first_order_lowpass(&meas_turn_vel_lp, 1.0/ MAIN_LOOP_HZ, 0.04);
    rc_filter_first_order_lowpass(&set_fwd_vel_lp, 1.0/ MAIN_LOOP_HZ, 0.04);
    rc_filter_first_order_lowpass(&set_turn_vel_lp, 1.0/ MAIN_LOOP_HZ, 0.04);
    rc_filter_first_order_lowpass(&set_left_lp, 1.0/ MAIN_LOOP_HZ, 0.04);
    rc_filter_first_order_lowpass(&set_right_lp, 1.0/ MAIN_LOOP_HZ, 0.04);
    //Init PID filter for fwd and turn vel
    fwd_vel_pid = rc_filter_empty(); turn_vel_pid = rc_filter_empty();
    rc_filter_pid(&fwd_vel_pid, fwd_vel_pid_params.kp, fwd_vel_pid_params.ki, fwd_vel_pid_params.kd, 1.0 / fwd_vel_pid_params.dFilterHz, 1.0 / MAIN_LOOP_HZ);
    rc_filter_pid(&turn_vel_pid, turn_vel_pid_params.kp, turn_vel_pid_params.ki, turn_vel_pid_params.kd, 1.0 / turn_vel_pid_params.dFilterHz, 1.0 / MAIN_LOOP_HZ);
    ...
    while (running)
    {
        ...
        //printf("\033[2A\r|OdomDiff = %7.3f",delta_go);
    }
}
...
{
    float l_duty; float r_duty;
    static float duty[2];

    left_sp = left_sp/WHEEL_RADIUS *60/(2* PI);
    right_sp = right_sp/WHEEL_RADIUS *60/(2*PI);
    //compute l_duty, r_duty
    if (left_sp > 0)
    {
        l_duty = SLOPE_L_POS*left_sp + INTERCEPT_L_POS;
    }
    else if (left_sp < 0)
    {
        l_duty = SLOPE_L_NEG*left_sp + INTERCEPT_L_NEG;
    }
    else
    {
        l_duty = 0;
    ...
    if (right_sp > 0)
    {
        r_duty = SLOPE_R_POS*right_sp + INTERCEPT_R_POS;
    }
    else if (right_sp < 0)
    {
        r_duty = SLOPE_R_NEG*right_sp + INTERCEPT_L_NEG;
    }
    else
    {
        r_duty = 0;
