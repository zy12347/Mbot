...
//not sure about this, maybe used to sort the frontiers by distance
                                            ...
                                            const MotionPlanner& planner)
{
    ...
    */
    ...
    //not sure about how to drive to somewhere else, not sure about what map uses
    //decide to use nearest accessable centroid as next pose
    
    // First, choose the frontier to go to
    // Initial alg: find the nearest one
    //goalPose.theta = robotPose.theta;//use start pose's theta as goal pose's theta
    // Returnable path
    int INTMAX = 10000;
    mbot_lcm_msgs::robot_path_t path;
    //mbot_lcm_msgs::robot_path_t tmp_path;
    path.utime = utime_now();
    path.path_length = 1;
    path.path.push_back(robotPose);
    int minLength = INTMAX;//max
    int unreachable_frontiers = 0;//not sure about unreachable
    //path = path_to_frontier(frontier,robotPose,map,planner);

    for(int i=0;i<frontiers.size();i++){
        mbot_lcm_msgs::robot_path_t  tmpPath = path_to_frontier(frontiers[i],robotPose,map,planner);
        if(tmpPath.path_length < minLength){
            path = tmpPath;
            minLength = tmpPath.path_length;
        }
    }
    //return a path to nearest accessable frontier's centroid
...
// implemention of find robot_path

mbot_lcm_msgs::robot_path_t path_to_frontier(const frontier_t& frontier,const mbot_lcm_msgs::pose_xyt_t& pose,const OccupancyGrid& map,const MotionPlanner& planner){
    Point<double> centroid = find_frontier_centroid(frontier);//use centroid to represent each frontiers
    //minCentroid = centroid;
    //pose.utime = utime_now();//not sure about how to set utime
    mbot_lcm_msgs::pose_xyt_t desiredPose = nearest_navigable_cell(pose,Point<float>(centroid.x, centroid.y),map,planner);
    mbot_lcm_msgs::robot_path_t path = planner.planPath(pose,desiredPose);//not A*
    //planner.planPath(robotPose,goal,param);//A*
    //if no accessable frontier, don't move
    return path;
}

mbot_lcm_msgs::pose_xyt_t nearest_navigable_cell(mbot_lcm_msgs::pose_xyt_t pose,Point<float> desiredPosition,const OccupancyGrid& map,const MotionPlanner& planner){
    mbot_lcm_msgs::pose_xyt_t goalPose = search_to_nearest_free_space(desiredPosition,map,planner);
    // mbot_lcm_msgs::pose_xyt_t path = planner.planPath(pose,goalPose);//not A*
    //mbot_lcm_msgs::pose_xyt_t path = planner.planPath(pose,goalPose,searchParam);//A*
    return goalPose;
}
...
mbot_lcm_msgs::pose_xyt_t search_to_nearest_free_space(Point<float> position,const OccupancyGrid& map,const MotionPlanner& planner){
    ...
    while(!cellQueue.empty()){
        ...
        {
            Point<int> neighbor(nextCell.x + xDeltas[n], nextCell.y + yDeltas[n]);
            if(map.logOdds(neighbor.x, neighbor.y) < 0)
            {
                Point<double> pos = grid_position_to_global_position(neighbor, map);
                pose.x = pos.x;
                pose.y = pos.y;
                pose.theta = 0;
