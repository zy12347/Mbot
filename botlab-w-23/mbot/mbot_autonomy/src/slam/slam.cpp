...
, odomResetThreshDist_(.05f) // .05
, odomResetThreshAng_(.08f)  // .08 ~5 degrees.
, mapFile_(mapFile)
...
bool OccupancyGridSLAM::isReadyToUpdate(void)
{
    ...
    if(!incomingScans_.empty())
    {
        ...
        //std::cout <<"Test: "<<nextScan.times.front() <<"\n";
...
void OccupancyGridSLAM::initializePosesIfNeeded(void)
{
    ...
    if(!haveInitializedPoses_)
    {
        ...
        //std::cout << global_position_to_grid_position(Point<double>(0,0), map_) << "\t";
        //std::cout << int(map_.logOdds(100,100)) << "\t";
        //std::cout << map_.cellsPerMeter() << "\t";
...
void OccupancyGridSLAM::updateMap(void)
{
    if(mode_ != localization_only && mode_ != action_only)
    {
        ...
        //std::cout << int(map_.logOdds(397,383)) << "\n";
        haveMap_ = true;
    }
